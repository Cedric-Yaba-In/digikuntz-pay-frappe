...
def load_default_gateway(ressource_to_pay,customer_data):
    ...
    if default_gateway == enum.GatyewayPaymentType.E_NKAP.value:
        enkapApiGateway = enkap.EnkapApiGateway(ressource_to_pay,customer_data,f"{constant.RETURN_SUCCESS_URL}&ref={ressource_to_pay.name}",f"{constant.RETURN_CANCEL_URL}&ref={ressource_to_pay.name}")
        return enkapApiGateway
    elif default_gateway == enum.GatyewayPaymentType.MTN_MOMO.value:
        mtn_momo_gateway = mtn_momo.MTNMoMoGateway(ressource_to_pay,customer_data,f"{constant.RETURN_SUCCESS_URL}&ref={ressource_to_pay.name}",f"{constant.RETURN_CANCEL_URL}&ref={ressource_to_pay.name}")
        return mtn_momo_gateway
    elif default_gateway == enum.GatyewayPaymentType.CINET_PAY.value:
        cinet_pay_gateway = cinet_pay.CinetPayApiGateway(ressource_to_pay,customer_data,f"{constant.RETURN_SUCCESS_URL}&ref={ressource_to_pay.name}",f"{constant.RETURN_CANCEL_URL}&ref={ressource_to_pay.name}")
